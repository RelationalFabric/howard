...
## Moving from ad-hoc boolean checks to first-class propositions. How Howard transforms the foundation of Software Architecture.

### The Category Error We Keep Making

I'll be honest: seeing a file named `UserValidator` or a function called `validateUser()` gives me the ick. I tolerate it at the boundaries—where we have to scrub 'dirty' input from an API or a form—but when I see it creeping into my core logic, it feels like the code itself has become dirty. It's a sign that we've failed to establish trust in our own internal structures.

But here's the problem we rarely articulate:

**...we are treating identity as a check when it should be a proof.**

Validation, type guards, and predicates in general are something we *do* to the data. A claim is something the data *possesses*. The difference isn't semantics, it's a category error. How we think about and approach our systems shapes how we architect them. When we treat correctness as a process we have to repeat, rather than a property the data carries with it, we end up building what I call 'infrastructures of suspicion.' We've all seen it: every single layer of the stack re-interrogating the data because it doesn't trust that the previous layer actually did its job.
...
As my colleagues over the years will tell you, I would call this "a failure of abstraction".
...
### The Principle That Guides My Thinking

I've always viewed the Curry-Howard Correspondence as more than just a profound piece of computer science history; for me, it has been a guiding principle in my approach to reasoning about software. It's the realisation that our types aren't just labels, and our functions aren't just procedures, they are propositions and proofs. When we ignore this, we aren't just writing 'messy' code; we are breaking the logical integrity of the system.

Building on earlier work by Haskell Curry, William Alvin Howard formulated the Curry-Howard correspondence, culminating in a paper that would change the way academics thought about software. As Xavier Leroy later reflected:
...
The difference is subtle but transformative. For a long time, my own mental model was the 'gatekeeper'—I felt like I had to be constantly suspicious of data, re-checking it at every turn. However, once I began to focus on claims, I started thinking like an archivist. Data arrives with its credentials already established; my job is simply to read them, not to re-interrogate the source.

This shift changes how you design APIs, structure modules, and reason about data flow. Functions stop asking "is this valid?" and start asking "what has been proven about this?" The defensive crouch relaxes into a confident composition.

The economics are stark: in an infrastructure of suspicion, you pay for the same check a thousand times across a thousand services. In a claim-based architecture, you pay for the proof once. This is the shift that finally cuts through the 'semantic noise' I've seen clogging up almost every distributed system I've worked on. We move from a world of hopeful assumptions to one of proven propositions.
...
Howard didn't start as an academic exercise. It started while I was architecting Filament, the meta-programming foundation of the Relational Fabric ecosystem. I was looking at how languages like Clojure handle dispatch through multimethods and protocols. They are powerful, but they feel disconnected—protocols give you speed by dispatching on the first argument's type, while multimethods give you expressivity by analysing the data more deeply.

I realised the only thing actually unifying them was the predicate.

I started experimenting: what if we could generate "tokens" for these predicates under the hood? If we could tokenise both the implementation and the predicate's outcome, we could build a routing table that allowed for massive optimisations, like reusing the outcomes of earlier predicates in higher-order ones.

But as I tried to build this "automatic memoisation," I realised I was fighting the abstraction. I didn't just need a utility; I needed a new layer in the software stack. I wanted first-class, composable propositions.

Then it hit me: this is exactly what the Curry-Howard Correspondence describes. And when it came to naming the library, I figured Haskell Curry had enjoyed enough of the spotlight. It was time to give William Alvin Howard his due.
...
In traditional code, when a complex validation fails, you get `false`. That's it. You're left to trace through nested conditionals, console-logging your way to the offending check. I've lost count of how many times I've had to debug one of these predicates by essentially writing an ad hoc, informally specified implementation of half of Howard's claims just to see what went wrong. Greenspun strikes again; we're just reinventing the engine we should have started with.
...
This is the debugging experience I spent years dreaming of: failures that actually have the decency to explain themselves.
...
Neither of these options sits right with me. You're forced to choose between sacrificing correctness or burying your performance in redundant logic. It's a lose-lose situation that shouldn't exist.
...
Most of us are drowning in ad-hoc validation debt, and we've just accepted it as the cost of doing business. I don't think it has to be. If you're tired of the 'Infrastructure of Suspicion,' I'd love for you to try Howard and help me build a better way to think about our data.
